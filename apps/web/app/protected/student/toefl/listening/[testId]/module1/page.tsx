...
export default function Module1Page() {
  ...
  const track = test?.hard?.tracks[trackIdx];
  ...
  const handleNext = () => {
    ...
    if (qIdx < track.questions.length - 1) {
      setQIdx(q => q + 1);
    } else if (trackIdx < (test?.hard?.tracks.length || 0) - 1) {
      setTrackIdx(t => t + 1);
      setQIdx(0);
    } else {
      submitModule1();
  ...
  const submitModule1 = async () => {
    let correct = 0;
    test?.hard?.tracks.forEach(t =>
      t.questions.forEach(q => {
        const idx = answers[q.id];
        if (idx !== undefined && q.choices[idx]?.correct) correct++;
      })
    );
    const rate = (correct / totalQuestions) * 100;
    const mode = rate >= 60 ? 'hard' : 'easy';
    ...
    router.push(`/student/toefl/listening/${testId}/module2?mode=${mode}`);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="border-b bg-white px-6 py-4 shadow-sm">
        <div className="mx-auto max-w-6xl flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-gray-900">Module 1</h1>
            <p className="text-xs text-gray-500">{trackIdx + 1}/{test.hard.tracks.length} · 문제 {Object.keys(answers).length}/{totalQuestions}</p>
          </div>
