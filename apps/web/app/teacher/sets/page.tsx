...
export default function Page() {
  ...
  const [sel, setSel] = useState<{ section: Section; set_id: string } | null>(null);
  ...
  const pick = (section: Section, set_id: string) => {
  ...
  const createNewVersion = async () => {
    ...
    try {
      ...
      try {
        ...
        alert('payload_json 이 올바른 JSON이 아닙니다.\n' + e.message);
        setBusy(false);
        return;
      }
  ...
  const createSet = async () => {
    const section = (prompt('SECTION (reading/listening/speaking/writing)', 'reading') ||
      'reading') as Section;
    const set_id = prompt('SET ID (e.g. TPO 1)', 'TPO 1') || 'TPO 1';
  ...
  return (
    ...
    >
            ...
            return (
              <div key={k} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 8, background: '#fafafa' }}>
                <div
                  ...
                  onClick={() => pick(section as Section, set_id)}
                >
                  <div>
                    <strong>{section}</strong> / {set_id}
                  </div>
                  <div style={{ fontSize: 12, color: '#666' }}>
                    {pub ? `published v${pub.version}` : 'unpublished'}
                  </div>
                </div>
                <div style={{ marginTop: 8, fontSize: 12, color: '#444' }}>
                  {list.slice(0, 3).map((x) => `v${x.version}`).join(', ')}
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 12, minHeight: 400 }}>
        {!sel ? (
          <div>왼쪽에서 세트를 선택하세요.</div>
        ) : (
          <>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
              <h3 style={{ margin: 0 }}>
                {sel.section} / {sel.set_id}
              </h3>
              <input
              ...
              />
              <button disabled={busy} onClick={createNewVersion}>
                {busy ? 'Saving...' : 'Save as New Version'}
              </button>
            </div>

            <div>
              <small>payload_json (예: {"{\"answer_key\":{\"1\":2,\"2\":3}}"})</small>
            </div>
            <textarea
              value={editor}
              ...
              <ul>
                {(groups[`${sel.section}::${sel.set_id}`] || []).map((r) => (
                  <li key={r.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span>
                      v{r.version} {r.published_at ? '(published)' : ''}
                    </span>
