...
export default async function ListeningReviewPage(props: any) {
  ...
  const supabase = await getSupabaseServer(); // ??await Ï∂îÍ?

  // ?∏ÏÖòID??bigint Í∏∞Ï? numberÎ°??¨Ïö©
  const sid = Number(params.sessionId);

  // ?êÏàò ??Ï§?  const { data: scoreRow, error: scoreErr } = await supabase
    .rpc('listening_review_score', { session_id: sid })
    .returns<Score>()
    .single();

  // Î¨∏Ìï≠Î≥?  const { data: rowsRaw, error } = await supabase
    .rpc('listening_review_rows', { session_id: sid })
    .returns<unknown>();

  // ?§Î•ò/ÎπÑÏñ¥?àÏùå ?Ä??(Î™®ÌÇπ)
  if (error || scoreErr || !rowsRaw || !scoreRow) {
    ...
    return (
      <div className="mx-auto space-y-6 pb-8 max-w-4xl">
  ...
  // ??rows ?ïÍ∑ú?? ??ÉÅ Row[]Î°?ÎßåÎì§?¥ÏÑú Table???ÑÎã¨
...
// ?∞Ì???Í∞Ä?? ÏµúÏÜå ?ÑÎìú(q_no) Í∏∞Ï??ºÎ°ú Row ?ïÌÉú?∏Ï? ?ïÏù∏
