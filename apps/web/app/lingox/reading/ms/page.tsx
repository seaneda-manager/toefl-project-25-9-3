...
export default async function Page() {
  ...
  if (!user) redirect("/auth/login?next=/lingox/reading/ms");
  ...
  return (
    <main className="mx-auto max-w-4xl p-6 space-y-4">
      <h1 className="text-2xl font-semibold">LingoX Reading · MS</h1>

      <div className="space-y-3">
        {sets.map((s) => (
          <Link
            key={s.id}
            href={`/lingox/reading/ms/test?setId=${encodeURIComponent(s.id)}`}
            className="block rounded-2xl border bg-white p-4 hover:bg-slate-50"
          >
            <div className="text-base font-semibold">{s.title || s.id}</div>
            <div className="mt-1 text-xs text-neutral-500">setId: {s.id}</div>
          </Link>
        ))}
      </div>
    </main>
