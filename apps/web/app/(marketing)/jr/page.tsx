...
import { JR_DEFAULT } from '@/components/landing/defaults';
...
async function getConfig(): Promise<LandingPageConfig> {
  try {
    ...
    if (!url || !key) return JR_DEFAULT;
    const supabase = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } });
    const { data } = await supabase.from('site_settings').select('value').eq('key', 'landing_jr_v1').maybeSingle();
    if (data?.value && typeof data.value === 'object') return data.value as LandingPageConfig;
  } catch { /* fallback */ }
  return JR_DEFAULT;
}

export default async function JrLandingPage() {
