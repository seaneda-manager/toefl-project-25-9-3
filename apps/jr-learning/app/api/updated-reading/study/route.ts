// apps/web/app/api/updated-reading/study/submit/route.ts
...
export async function POST(req: NextRequest) {
  ...
  if (serr || !session) {
    return NextResponse.json({ ok: false, error: serr?.message ?? 'session insert failed' }, { status: 500 });
  }

  const sessionId = session.id as string;

  // 3) (옵션) 여기서 review_items, vocab_items 자동 생성은 나중에 정교화
  //    지금은 sessionId만 반환해서 클라이언트가 /updated-reading/review/[sessionId]로 이동하게.
